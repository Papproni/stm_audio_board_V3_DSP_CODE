...
void SAB_lpf_delete( SAB_lpf_tst* self){
    // TODO
}

// Process Function for SAB_lpf_tst
void SAB_lpf_init( SAB_lpf_tst* self){

    strcpy(self->intercom_fx_data.name, "lpf");
...
// Process Function for SAB_lpf_tst
float32_t SAB_lpf_process( SAB_lpf_tst* self, float32_t input_f32){
    // Freq
    self->param_1_value = conv_raw_to_param_value(self->intercom_parameters_aun[0].value_u8,50, 10000);
    // Q
    self->param_2_value = conv_raw_to_param_value(self->intercom_parameters_aun[1].value_u8,0.1, 20);
    // Gain
    self->param_3_value = conv_raw_to_param_value(self->intercom_parameters_aun[2].value_u8,0.001, 5);
    // Generated outputs from other Jinja templates
    
    self->input_block_block0_output_f32 = input_f32;
    
    // Biquad filter process function
calculate_biquad_coeffs(self->biquad_filter_block_block2_coeffs_af32,LPF, self->param_1_value, self->param_2_value, 48000);
...
return self->output_block_block1_input_f32;
