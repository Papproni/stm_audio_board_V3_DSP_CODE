...
float32_t SAB_fuzz_process( SAB_fuzz_tst* self, float input_f32){
	self->gain_f32 		= conv_raw_to_param_value(self->intercom_parameters_aun[1].value_u8,0, 20);
	self->vol_f32		= conv_raw_to_param_value(self->intercom_parameters_aun[0].value_u8,0, 4);
	...
	output_f32 = output_f32*self->vol_f32;
